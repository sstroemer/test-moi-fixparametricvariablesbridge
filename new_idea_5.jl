    ...
    for (vars, coeff) in terms
        ...
        if v_a !== nothing
            if !haskey(updates, vars.b)
                ...
                updates[vars.b] = AffineVariableUpdate(get(affine.terms, vars.b, 0.0), spzeros(n_param))
            end
        ...
        elseif v_b !== nothing
            if !haskey(updates, vars.a)
                updates[vars.a] = AffineVariableUpdate(get(affine.terms, vars.a, 0.0), spzeros(n_param))
            end
...
function enable_parameters!(model::JuMP.Model)
    ...
    model.ext[:__parameters] = OrderedDict{VariableRef, ParameterData}()
